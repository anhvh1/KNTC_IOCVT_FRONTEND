...
const DMTrangThaiDon = (props) => {
  ...
  const deleteModalAddEdit = (TrangThaiDonID) => {
    Modal.confirm({
      ...
      onOk: () => {
        ...
        api
          .XoaTrangThaiDon(TrangThaiDonID)
          .then((res) => {
  ...
  const showModalEdit = (id) => {
    ...
    setAction('edit');
    api
      .ChiTietTrangThaiDon({TrangThaiDonID})
      .then((res) => {
        if (res.data.Status > 0) {
    ...
    return (
      <div className={'action-btn'}>
        {role.edit ? (
          <Tooltip title={'Sửa'}>
            <EditOutlined
              onClick={() => showModalEdit(record.TrangThaiDonID)}
            />
          </Tooltip>
        ) : (
          ''
        ...
        {role.delete ? (
          <Tooltip title={'Xóa'}>
            <DeleteOutlined
              onClick={() => deleteModalAddEdit(record.TrangThaiDonID)}
            />
          </Tooltip>
        ) : (
          ''
...
function mapStateToProps(state) {
  return {
    ...
    role: getRoleByKey(state.Auth.role, 'danh-muc-trang-thai-don'),
  };
}
