...
import actions from "../../../redux/DanhMuc/DMChucVu/actions";
...
const DMChucVu = (props) => {
  ...
  const deleteModalAddEdit = (ChucVuID) => {
    Modal.confirm({
      title: "Xóa Dữ Liệu",
  ...
  const renderThaoTac = (record) => {
    return (
      <div className={"action-btn"}>
        {role?.edit ? (
          <Tooltip title={"Sửa"}>
            <EditIcon onClick={() => showModalEdit(record.ChucVuID)} />
          </Tooltip>
        ) : (
          ""
        )}
        {role?.delete ? (
          <Tooltip title={"Xóa"}>
            <DeleteIcon onClick={() => deleteModalAddEdit(record.ChucVuID)} />
          </Tooltip>
        ) : (
          ""
        )}
  ...
  const { DanhSachChucVu, TotalRow, role } = props;
      ...
      <Box>
        <BoxFilter>
          ...
          <InputSearch
            ...
            placeholder={"Nhập mã hoặc tên chức vụ"}
            style={{ width: 300 }}
...
export default connect(mapStateToProps, actions)(DMChucVu);
