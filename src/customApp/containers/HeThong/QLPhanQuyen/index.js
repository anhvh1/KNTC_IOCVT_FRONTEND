...
import BreadCrumb from "../../NghiepVu/Shared/Component/BreadCumb.js";
...
import Setting from "../../../../components/utility/Setting";
...
const QLPhanQuyen = (props) => {
  ...
  const role = useSelector((state) =>
    getRoleByKey(state.Auth.role, "phan-quyen")
  );
  ...
  const submitModalAddUser = (data) => {
    ...
    setLoading(true);
    api
      .themNhieuNguoiDung({ ...data, DanhSachNguoiDungID, TenNguoiDung: "" })
      .then((response) => {
        setLoading(false);
        if (response.data.Status > 0) {
          //message success
                ...
                <Col lg={12} md={24}>
                  <Box className="box_class">
                    <div className="action_class">
                      ...
                      {role.add ? (
                        <Button type="add" onClick={openModalAddUser}>
                ...
                <Col lg={12} md={24}>
                  <Box className="box_class">
                    <div className="action_class">
                      ...
                      {role.edit ? (
                        <Button type="add" onClick={openModalAddPermission}>
      ...
      <div className={"action-btn"}>
        {role.edit ? (
          <Tooltip title={"Cấu hình"}>
            <Setting onClick={() => onSelectChange(record.NhomNguoiDungID)} />
          </Tooltip>
        ) : (
          ""
        )}
        ...
        {role.delete ? (
          <Tooltip title={"Xóa"}>
            <DeleteIcon onClick={() => deleteGroup(record.NhomNguoiDungID)} />
          </Tooltip>
        ) : (
          ""
        )}
