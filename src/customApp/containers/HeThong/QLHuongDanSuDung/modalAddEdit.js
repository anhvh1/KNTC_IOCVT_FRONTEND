...
const ModalAddEdit = (props) => {
  ...
  const GetListMenuByCap = (CapID) => {
    api.danhSachCacCapDonVi({CapID}).then((res) => {
  ...
  useEffect(() => {
    ...
    if (dataModalAddEdit) {
      ...
      let listOption = [];
      // ListSideBar?.forEach((groupItem) => {
      //   let group = {
      //     key: groupItem.MaMenu,
      //     label: groupItem.TenMenu,
      //     Children: [],
      //   };
      //   group.Children =
      //     (groupItem.Children &&
      //       groupItem.Children.map((optionItem) => {
      //         let disabled = false;
      //         DanhSachHuongDan?.forEach((itemHD) => {
      //           if (itemHD.MaChucNang === optionItem.MaMenu) disabled = true;
      //         });
      //         if (optionItem.MaMenu !== 'quan-ly') {
      //           return {
      //             key: optionItem.MaMenu,
      //             label: optionItem.TenMenu,
      //             hienthi: optionItem.HienThi,
      //             disabled,
      //           };
      //         }
      //       })) ||
      //     [];
      //   listOption.push(group);
      // });
