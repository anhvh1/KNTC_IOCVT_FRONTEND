...
const QLHuongDanSuDung = (props) => {
  document.title = "Quản lý hướng dẫn sử dụng";
  ...
  const deleteData = (HuongDanSuDungID) => {
    Modal.confirm({
      title: "Xóa dữ liệu",
      content: " Bạn có muốn xóa file hướng dẫn sử dụng này không?",
      cancelText: "Không",
      okText: "Có",
      onOk: () => {
        api
    ...
    },
    {
      ...
      render: (text, record, index) => {
        return (
          <span>
            {record.CapID
              ? DanhSachCacCap?.find((item) => item.Cap === record.CapID)
                  ?.TenCap
              : null}
          </span>
        );
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <div className="action-btn">
          {role.edit ? (
            <EditIcon onClick={() => showModalEdit(record)} />
          ) : null}
          {role.delete ? (
            <DeleteIcon onClick={() => deleteData(record.HuongDanSuDungID)} />
          ) : null}
