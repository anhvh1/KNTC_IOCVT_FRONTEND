...
export default (props) => {
  ...
  console.log("filteredDoiTuong", filteredDoiTuong);
  useEffect(() => {
    GetCongBoKL();
    GetListDoiTuong();
  }, []);
  ...
  const onOk = async (e) => {
    ...
    const formattedValue = {
      ...
      Child: soLieu.map((item) => ({
        // This will now work as soLieu is an array
        CapNhapSoLieuID: 0,
  ...
  const handleChange = (key, dataIndex, value) => {
    ...
    if (target) {
      ...
      if (dataIndex === "loaiDoiTuong") {
        ...
        const filtered = (DoiTuong || []).filter(
          (item) => item.LoaiDoiTuong === parseInt(value)
        ); // Added null check
    ...
    >
      ...
      >
        ...
        <Item
          ...
          wrapperCol={{ span: 24 }}
        >
          <Select allowClear name={"HuyenID"} listHeight={100}>
            {CongBoKLID?.map((item) => (
              <Option key={item.ID} value={item.ID}>
                {item.SoVanBan}
              </Option>
