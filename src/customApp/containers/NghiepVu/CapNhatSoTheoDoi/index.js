...
import actions from "../../../../customApp/redux/NghiepVu/CapNhatSoTheoDoi/actions";
...
import LayersHalfIcon from "../../../../components/utility/LayersHalfIcon";
...
const CapNhatSoTheoDoi = (props) => {
  document.title = " Phân công theo dõi xử lý sô liệu sau thanh tra";
  const { TotalRow, TableLoading, ListSyntheticPlan } = useSelector(
    (state) => state.CapNhatSoTheoDoi
  );
  console.log("ListSyntheticPlan", ListSyntheticPlan);
  ...
  const defaultDoiTuongTT = filterData?.DoiTuongTT
    ...
    : DONVIHANHCHINH;
  console.log("defaultDoiTuongTT", defaultDoiTuongTT);
  ...
  const [dataModalRaSoat, setDataModalRaSoat] = useState({});
  const [visibleModalRaSoat, setVisibleModalRaSoat] = useState(false);
  const [modalKey, inceaseModalKey] = useKey();
  const hideModalAddEdit = () => {
    // setSelectedRowsKey([]);
    setDataModalRaSoat({});
    setVisibleModalRaSoat(false);
  };
  const showModalRaSoat = (CuocThanhTraID) => {
    api
  ...
  const dowloadInspection = async () => {
    ...
    try {
      exportExcel(
        ...
        " Phân công theo dõi xử lý sô liệu sau thanh tra.xlsx",
        styleWrapper
      );
  ...
  const renderThaoTac = (record) => {
    return (
      <div className={"action-btn"}>
        <Tooltip title={"Rà soát hồ sơ"}>
          <LayersHalfIcon
            onClick={() => {
              showModalRaSoat(record.CuocThanhTraID);
              setCuocThanhTraID(record.CuocThanhTraID);
            }}
          />
        </Tooltip>
        {/* {role && role.edit && ( */}
        <Tooltip title={"Cập nhật số liệu"}>
  ...
  return (
    <LayoutWrapper>
      ...
      <Wrapper>
        <Box>
          <div
            ...
            className="info"
          >
            Danh sách thành viên được phân công theo dõi xử lý sô liệu sau thanh
            tra
          </div>
      ...
      />
      <ModalRaSoat
        visible={visibleModalRaSoat}
        dataEdit={dataModalRaSoat}
        key={modalKey}
        onCancel={hideModalAddEdit}
        CuocThanhTraID={CuocThanhTraID}
        setDataModalRaSoat={setDataModalRaSoat}
        ListHinhThuc={ListHinhThuc}
        ListYear={ListYear}
        ListAgency={ListAgency}
      />
...
export default CapNhatSoTheoDoi;
