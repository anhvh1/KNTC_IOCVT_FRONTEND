...
export default (props) => {
  ...
  const [dataSource, setDataSource] = useState(() => {
    ...
    if (props.dataEdit && props.dataEdit.length > 0) {
      return [
        emptyFirstRow,
        ...props.dataEdit.flatMap((item, index) => ( // Use flatMap to handle Child array
  ...
  console.log("dataSource789", dataSource)
  useEffect(() => {
    if (DoiTuong) { // Check if DoiTuong is defined
  ...
  return (
    <Modal
      title={`${action === "editSoLieu" ? "Cập nhật" : "Thêm"} nội dung xử lý kỷ luật`}
