...
export default (props) => {
  ...
  const { dataEdit, loading, visible, action, CuocThanhTraID, dataModalEditTienDat, capNhapLoaiSoLieuID } = props;
  console.log("dataModalEditTienDat", dataModalEditTienDat)
  useEffect(() => {
    if (dataModalEditTienDat) {
      form &&
        form.setFieldsValue({
          ...dataModalEditTienDat,
          NgayBanHanh: dataModalEditTienDat.NgayBanHanh ? dayjs(dataModalEditTienDat.NgayBanHanh) : null,
          ThoiHan: dataModalEditTienDat.ThoiHan ? dayjs(dataModalEditTienDat.ThoiHan) : null,
  ...
  const onOk = async (e) => {
    ...
    const formattedValue = {
      ...
      DeleteIDs: DeleteIDs,
      NgayBanHanh: value.NgayBanHanh ? dayjs(value.NgayBanHanh).format('YYYY-MM-DD') : null,
  ...
  useEffect(() => {
    const filteredDoiTuong = dataSource.map((item) =>
      DoiTuong.filter(doiTuong => doiTuong.LoaiDoiTuong === item.loaiDoiTuong)
    );
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'DoiTuongID',
      render: (_, record, index) => ( // Thêm index vào đây
        <Select
          style={{ width: '100%' }}
        ...
        >
          {filteredDoiTuong[index]?.map((item) => ( // Sử dụng index thay vì record.key
            <Option key={item.ID} value={item.ID}>
              {item.Ten}
            </Option>
          )) || null} {/* Kiểm tra null để tránh lỗi */}
        </Select>
      ),
    },
    {
      ...
      children: [
        {
          ...
          render: (_, record) => (
            <Input
              ...
              onChange={(e) => handleChange(record.key, 'SoTienPhaiThu', e.target.value)}
            />
          ),
        },
        {
      ...
      children: [
        {
          ...
          render: (_, record) => (
            <Input
              ...
              onChange={(e) => handleChange(record.key, 'SoDatPhaiThu', e.target.value)}
            />
          ),
        },
        {
      ...
      render: (_, record, index) => (
        <Space size="middle" style={{ display: 'flex', justifyContent: 'center' }}>
          {record.key === dataSource[0].key && (
            ...
            // </svg>
            <svg onClick={() => {
              if (!dataSource[0].loaiDoiTuong || !dataSource[0].DoiTuongID || !dataSource[0].SoTienPhaiThu || !dataSource[0].SoDatPhaiThu) {
  ...
  const handleDelete = (key) => {
    ...
    if (itemToDelete && itemToDelete.CapNhapSoLieuID) {
      setDeleteIDs(prevDeleteIDs => [...prevDeleteIDs, itemToDelete.CapNhapSoLieuID]);
    }
  ...
  return (
    <Modal
      title={`${action === "editSoLieu" ? "Cập nhật" : "Thêm"} quyết định xử lý thu hồi`}
