...
const FormGeneralInspectionInfo = ({ CuocThanhTraID }) => {
  ...
  const fetchData = () => {
    api
      .GetListDoiTuong(CuocThanhTraID, 16)
      .then((res) => {
  ...
  const handleAddRow = () => {
    ...
    console.log("newLoaiDoiTuong:", newLoaiDoiTuong);
  };
  ...
  const handleAddRowSua = (newTenSua1, newLoaiDoiTuongSua1, ID) => {
    ...
    api
      .Update({ Ten, LoaiDoiTuong, TypeChucNang: 16, CuocThanhTraID, ID })
      .then((res) => {
        if (res.data.Status > 0) {
          message.success(res.data.Message);
          fetchData();
    ...
    console.log("newLoaiDoiTuongSua", newLoaiDoiTuongSua);
  };
  ...
  return (
    <div style={styles.formImportContainer}>
      ...
      >
        2. Danh sách đối tượng sai phạm liên quan kết luận thanh tra, kiểm tra
      </h2>
      <BoxTable
        columns={columns}
