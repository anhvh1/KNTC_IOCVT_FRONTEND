...
class ModalAddEditHoSoTaiLieu extends React.Component {
  ...
  deteleFile = (item, index) => {
    const {ListFileDinhKem, FileDinhKemDelete} = this.state;
    if (ListFileDinhKem.length > 1) {
      //   ModalAnt.confirm({
      //     title: 'Thông báo',
      //     content: 'Bạn có muốn xóa file đính kèm cuối cùng này không?',
      //     okText: 'Có',
      //     cancelText: 'Không',
      //     onOk: () => {
      //       item.TrangThai = 0;
      ...
      //       if (item.FileID) {
      //         const newFileDinhKemDelete = [...FileDinhKemDelete];
      //         newFileDinhKemDelete.push(item.FileID);
      //         this.setState({FileDinhKemDelete: newFileDinhKemDelete});
      //       }
      //       this.setState({
      ...
      if (item.FileID) {
        ModalAnt.confirm({
          ...
          onOk: () => {
            ...
            const newFileDinhKemDelete = [...FileDinhKemDelete];
            if (item.FileID) {
              newFileDinhKemDelete.push(item);
            }
            this.setState({
              ListFileDinhKem: newArr,
              isFalseFile: false,
              messErr: '',
              FileDinhKemDelete: newFileDinhKemDelete,
            });
          },
        });
      } else {
      ...
      message.destroy();
      message.warning('Đã là file cuối cùng, không thể xóa');
    }
  };
    ...
    this.formRef.current
      ...
      .then((FormValue) => {
        const value = {
          ...
          FileDinhKemDelete: FileDinhKemDelete,
  ...
  render() {
    const {dataEdit, visible, fileKey, onCancel} = this.props;
      ...
      >
        <Wrapper>
          <Form ref={this.formRef}>
            ...
            {isFile ? (
              <p style={{color: 'red', marginLeft: '5px', textAlign: 'right'}}>
