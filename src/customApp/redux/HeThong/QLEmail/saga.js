...
function* getList({payload}) {
  try {
    ...
    yield put({
      type: actions.QUANLYEMAIL_GET_LIST_REQUEST_SUCCESS,
      payload: {
        DanhSachEmail: response.data.Data,
        TotalRow: response.data.TotalRow,
      },
    });
  } catch (e) {
    yield put({
      type: actions.QUANLYEMAIL_GET_LIST_REQUEST_ERROR,
    });
  }
}
...
export default function* rootSaga() {
  ...
  yield all([yield takeEvery(actions.QUANLYEMAIL_GET_LIST_REQUEST, getList)]);
}
