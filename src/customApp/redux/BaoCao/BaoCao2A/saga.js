...
function* getThongTinBaoCao({payload}) {
  try {
    ...
    yield put({
      type: actions.GETBAOCAO2A_INIT_REQUEST_SUCCESS,
      payload: {
    ...
    yield put({
      type: actions.GETBAOCAO2A_INIT_REQUEST_ERROR,
    });
  }
}
...
export default function* rootSaga() {
  yield all([
    yield takeEvery(actions.GETBAOCAO2A_INIT_REQUEST, getThongTinBaoCao),
  ]);
}
