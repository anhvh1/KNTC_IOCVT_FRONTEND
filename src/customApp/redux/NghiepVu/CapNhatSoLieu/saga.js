...
import api from "../../../containers/NghiepVu/CapNhatSoLieu/config";
...
function* getInit({ payload }) {
  try {
    ...
    yield put({
      type: actions.CAPNHATSOLIEU_GET_INIT_REQUEST_SUCCESS,
      payload: {
    ...
    yield put({
      type: actions.CAPNHATSOLIEU_GET_INIT_REQUEST_ERROR,
    });
  }
}
...
function* getList({ payload }) {
  try {
    ...
    yield put({
      type: actions.CAPNHATSOLIEU_GET_LIST_REQUEST_SUCCESS,
      payload: {
    ...
    yield put({
      type: actions.CAPNHATSOLIEU_GET_LIST_REQUEST_ERROR,
    });
  }
}

export default function* rootSaga() {
  yield all([yield takeEvery(actions.CAPNHATSOLIEU_GET_INIT_REQUEST, getInit)]);
  yield all([yield takeEvery(actions.CAPNHATSOLIEU_GET_LIST_REQUEST, getList)]);
}
