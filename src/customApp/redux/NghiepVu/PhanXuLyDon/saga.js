...
import api from '../../../containers/NghiepVu/PhanXuLyDon/config';
...
function* getInitData({payload}) {
  try {
    ...
    yield put({
      type: actions.PHANXULYDON_GET_INIT_REQUEST_SUCCESS,
      payload: {
    ...
    yield put({
      type: actions.PHANXULYDON_GET_INIT_REQUEST_ERROR,
    });
  }
}
...
function* getDanhSachSoTiepCongDan({payload}) {
  try {
    ...
    yield put({
      type: actions.PHANXULYDON_GET_LIST_REQUEST_SUCCESS,
      payload: {
    ...
    yield put({
      type: actions.PHANXULYDON_GET_LIST_REQUEST_ERROR,
    });
  }
}
...
export default function* rootSaga() {
  yield all([
    yield takeEvery(
      actions.PHANXULYDON_GET_LIST_REQUEST,
      getDanhSachSoTiepCongDan,
    ),
    yield takeEvery(actions.PHANXULYDON_GET_INIT_REQUEST, getInitData),
  ]);
}
