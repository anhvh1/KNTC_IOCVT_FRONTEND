...
const Sidebar = (props) => {
  ...
  useEffect(() => {
    ...
    console.log(ListSideBar, 'listSidebar');
    console.log(app.openKeys, 'app.openKeys');
  }, []);
  ...
  useEffect(() => {
    // const MenuChildActive = document.getElementById('wrapper-child__active');
    // const MenuChildUL = document.querySelector(
    //   '#wrapper-child__active ul:first-child',
    // );
    // const maxHeight = window
    //   .getComputedStyle(MenuChildActive, null)
    //   .getPropertyValue('max-height');
    // MenuChildActive.addEventListener('scroll', (e) => {
    //   if (MenuChildUL.clientHeight > Number(MenuChildActive.clientHeight)) {
  ...
  const handleScrollBar = () => {
    ...
    if (
      MenuChildUL.clientHeight > Number(MenuChildActive.clientHeight) &&
      !visibleBow.bowDown
    ) {
      setVisibleBow({
        bowDown: true,
      ...
      MenuChildUL.clientHeight < Number(MenuChildActive.clientHeight) &&
      (visibleBow.bowDown || visibleBow.bowUp)
    ) {
      setVisibleBow({
        bowDown: false,
        bowUp: false,
  ...
  const handleClickedSidebarNotChild = (MaMenu) => {
    setMaRedirect(MaMenu);
    dispatch(changeMaMenuActive(MaMenu));
    dispatch(changeListChild([]));
    // setListChildMenuActive([]);
  };

  const handleClickedSidebar = (MaMenu, ListChildMenuActive) => {
    const {changeListChild} = props;
    handleRedirect(ListChildMenuActive[0]?.MaMenu);
    handleRenderListChildActive(ListChildMenuActive);
    dispatch(changeMaMenuActive(MaMenu));
    // setListChildMenuActive(ListChildMenuActive);
    dispatch(changeListChild(ListChildMenuActive));
  };

  const handleCheckInitScrollBottom = (wrapperElement) => {
    const ListItemMenuChild = wrapperElement.children;
    let totalHeightItem = 0;
    for (let i = 0; i < ListItemMenuChild.length; i++) {
      if (ListItemMenuChild[i].nodeName !== 'DIV') {
        totalHeightItem += ListItemMenuChild[i].clientHeight;
      }
    }
    const maxHeightCurrentMenu = window
      .getComputedStyle(wrapperElement)
      .getPropertyValue('max-height');
    const NumberMaxHeight = Number(
      maxHeightCurrentMenu.toString().replace('px', '').trim(),
    );
    if (NumberMaxHeight < Number(totalHeightItem) && !visibleBow.bowDown) {
      setVisibleBow({bowUp: false, bowDown: true});
    } else if (
      NumberMaxHeight < Number(totalHeightItem) &&
      visibleBow.bowDown
    ) {
      setVisibleBow({bowUp: false, bowDown: false});
    }
  };

  const handleSrollMenuChild = (itemMenuChild) => {
    const pointCheckUp = itemMenuChild.querySelectorAll('.bow-up__menu')[0];
    const pointCheckDown = itemMenuChild.querySelectorAll('.bow-down__menu')[0];
    const scrollTo = (element, enitries) => {
      if (enitries === 'bottom') {
        element.scrollTo({top: element.scrollHeight, behavior: 'smooth'});
      } else if (enitries === 'top') {
        element.scrollTo({top: 0, behavior: 'smooth'});
      }
    };
    if (pointCheckUp) {
      pointCheckUp.addEventListener('click', (e) => {
        scrollTo(itemMenuChild, 'top');
      });
    }
    if (pointCheckDown) {
      pointCheckDown.addEventListener('click', (e) => {
        scrollTo(itemMenuChild, 'bottom');
      });
    }
  };
