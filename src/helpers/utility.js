...
export function getListYear() {
  ...
  if (dataConfig) {
    const { NamThanhtra } = dataConfig;
    const arrayYear = NamThanhtra?.split(",").map((item) => {
      return {
        id: item,
        name: item,
      };
...
export const mapFileToPromiseArray = (
  ListFile,
  PromiseArray,
  LoaiFile,
  api,
...
export const disabledDate = (current, nextDate, type) => {
  ...
  if (type === 2) {
    return dayjs(current, "YYYY/MM/DD") > nextDate.valueOf();
  }
  // console.log(current.valueOf(), 'current.valueOf()');
  // console.log(nextDate, 'nextDate');
  return current && current.valueOf() > dayjs(nextDate, "YYYY/MM/DD");
};

export const removePropertyNotValue = (object) => {
...
export const removeStringToStringNum = (string) => {
  if (string) {
...
export const formatValueSendRequest = (value) => {
...
export const formatNoiDung = (value, maxLength = 120) => {
  if (value) {
    // Tách chuỗi thành một mảng các từ
    const words = value.split(" ");

    // Kiểm tra nếu số lượng từ vượt quá giới hạn
    if (words.length > maxLength) {
      // Cắt mảng từ theo giới hạn và ghép lại thành chuỗi, thêm dấu "..."
      return (
        <Tooltip arrow={false} title={value} placement="rightTop">
          <p>{words.slice(0, maxLength).join(" ") + "..."}</p>
        </Tooltip>
      );
    }

    // Nếu không vượt quá giới hạn, trả về chuỗi ban đầu
    return value;
  }
  return value;
};

export const renderTrangThaiColor = (TrangThai) => {
  if (TrangThai) {
    const handleCheckColor = (TrangThai) => {
      switch (TrangThai) {
        case "Dự kiến":
          return "rgba(0, 128, 0, 1)";
        case "Chỉnh sửa, bổ sung":
        case "Chỉnh sửa":
          return "rgba(255, 165, 0, 1)";
        case "Đã lập kế hoạch":
          return "rgba(135, 206, 235, 1)";
        case "Đang rà soát":
          return "rgba(33, 111, 202, 1)";
        case "Đã rà soát":
          return "rgba(0, 0, 255, 1)";
        case "Kế hoạch chính thức":
          return "rgba(202, 67, 33, 1)";
        default:
          break;
      }
    };

    const color = handleCheckColor(TrangThai);

    return (
      <p
        className="hightlight-status"
...
export function formatNumberStr(num) {
  ...
  if (numberNum) {
    ...
    const formattedNumber = numberNum.toLocaleString("en-EN", options);
...
export const checkFieldsValidator = (
  ...
  if (value?.length > length) {
    callback(`${label} không được dài quá ${length} ký tự`);
  }
  console.log(isNotCheckAcent, "isNotCheckAcent");
  if (name !== "file" && !isNotCheckAcent) {
    if (name === "Email") {
      if (value && hasSpecialCharacters(value, "email")) {
        callback(`${label} không thể chứa ký tự đặc biệt`);
      }
    } else {
...
export function formatDateForm(date) {
  return date ? dayjs(date, "DD/MM/YYYY").format("MM/DD/YYYY") : null;
}
...
export const formatDataTreeSelect = (Data) => {
  const renderChildrenTreeSelect = (children, indexRoot) => {
    return children.map((value, index) => {
      let title = value.TenCoQuan;
      let label = `${value.TenCoQuan}`;
      let key = `${indexRoot}-${index}-${value.CoQuanID}`;
      let valueSelect = value.CoQuanID;
      let children =
...
export const formatTreeDataCoQuanV2 = (Data) => {
  let expandedKeys = [];
  const renderChildrenTreeSelect = (children, indexRoot) => {
    return children.map((value, index) => {
      let title = value.TenCoQuan;
...
export function exportExcel(rawHtml, exportName, styles) {
  let html, link, blob, url;
  let preHtml = `<html><head><meta charset='utf-8'><style>${styles}</style></head><body>`;
...
export function getLocalKey(properties, defaultValue) {
  const localKey = JSON?.parse(localStorage.getItem(properties));
...
export const handleRenderTrangThai = (record, name) => {
  if (record) {
    return record?.CheckTrangThai ? record?.TrangThaiMoi : record[name];
  }
};
// export const removeAscent2 = (str) => {
//   if (str === null || str === undefined) return str;
...
export const handleRenderTenNguonDonDen = (data) => {
  if (data) {
    return <p>{data?.TenNguonDonDen}</p>;
  }
};
